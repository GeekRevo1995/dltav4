...
class Client {
    ...
    handleMessage(a) {
        ...
        switch (d) {
            ...
            case 69:
                ...
                if (this.ghostCellsStep == 1 && this.ghostCells[0]) {
                    this.emit('gh', this);
                }
    ...
    calcQuadrant(a, b) {
        ...
        var c = 150;
        var d = a < this.mapMidX + c && a > this.mapMidX - c || b < this.mapMidY + c && b > this.mapMidY - c ? 4 : a >= this.mapMidX && b < this.mapMidY ? 0 : a < this.mapMidX && b < this.mapMidY ? 1 : a < this.mapMidX && b >= this.mapMidY ? 2 : 3;
        return d;
    }
    flushCellsData() {
        this.isFreeSpectate = false;
    ...
    heuristicalTopPlayerCells() {
        if (!this.leaderboard.length || !this.ghostCells.length) return []; // empty server?
        const topPlayerNick = this.leaderboard[0].nick;
        const topPlayerTotalMass = this.ghostCells[0].mass;
        const topPlayerViewCenter = [this.ghostCells[0].x, this.ghostCells[0].y];

        let potentialTopPlayerCells = this.cells.filter(cell => cell.targetNick === topPlayerNick);
        let potentialTopPlayerCellGroupsByAccount = Object.values(potentialTopPlayerCells.reduce(
            (p, c) => {
                // player cells should always have an accountID
                if (c.accountID !== null) {
                    if (p.hasOwnProperty(c.accountID))
                        p[c.accountID].push(c);
                    else p[c.accountID] = [c];
                }
                return p;
            }, {}
        ));

        // Can't see any top player cells. This happens e.g., when spectator has just been activated,
        // but the server still hasn't sent data for the top player's view.
        if (potentialTopPlayerCellGroupsByAccount.length === 0) return [];

        // This should happen almost always
        if (potentialTopPlayerCellGroupsByAccount.length === 1)
            return potentialTopPlayerCellGroupsByAccount[0];

        // Multiple players have the same name, and are in vision. Start best-guessing.
        // The primary factors here are total mass and center view. If both are too close to what
        // the top player should have, i don't know if there is a way to clearly determine
        // which group to take.

        let potentialTopPlayerCellGroupsByAccountWithMetadata = potentialTopPlayerCellGroupsByAccount.map(
            cellGroup => {
                let massDifference = Math.abs(cellGroup.reduce((p, c) => p + c.mass, 0) - topPlayerTotalMass);
                let [groupViewX, groupViewY] = cellGroup
                    .reduce(([xSum, ySum], { x, y }) => [xSum + x, ySum + y], [0, 0])
                    .map(e => e / cellGroup.length);
                let centerOfViewDistance = Math.sqrt((groupViewX - this.viewX) ** 2 + (groupViewY - this.viewY) ** 2);

                return [
                    cellGroup,
                    massDifference,
                    centerOfViewDistance
                ]
            }
        );

        // This is arbitrarily chosen - how much important does accuracy in distance have compared to mass?
        const weightFunc = (massDifference, centerOfViewDistance) => centerOfViewDistance + massDifference / 10;
        let potentialTopPlayerCellGroupsByAccountWeighted = potentialTopPlayerCellGroupsByAccountWithMetadata.map(
            ([cellGroup, massDifference, centerOfViewDistance]) => [cellGroup, weightFunc(massDifference, centerOfViewDistance)]
        );

        // return one with the lowest weight
        return potentialTopPlayerCellGroupsByAccountWeighted.reduce(
            (p, c) => c[1] < p[1] ? c : p
        )[0];
    }
    updateBound() {
    ...
    calcViewport() {
        var size = 0
        var mtp = AGAR_CONST.VIEWPORT_MULTIPLIER.DEFAULT; //1.995

        if (this.playerCells.length > 0)
            for (let cell of this.playerCells) size += cell.size;
        else if (this.isFreeSpectate === false && this.isSpectateEnabled === true)
            for (let cell of this.heuristicalTopPlayerCells()) size += cell.size
        else if (this.isFreeSpectate && this.isSpectateEnabled)
            mtp = AGAR_CONST.VIEWPORT_MULTIPLIER.FREE_SPECTATE;

        var s = Math.pow(Math.min(64 / size, 1), 0.4000);
        this.viewportW2s = AGAR_CONST.VIEWPORT_BASE_WIDTH / s / 2 * mtp; //WSVGA
        this.viewportH2s = AGAR_CONST.VIEWPORT_BASE_HEIGHT / s / 2 * mtp;
    }
    isInViewport(a, b, c) {
    ...
    circleInViewport(a, b, c) {
        ...
        var f = Math.abs(a - this.viewX);
        var g = Math.abs(b - this.viewY);
        if (f > this.viewportW2s + c) {
            return false;
        }
        if (g > this.viewportH2s + c) {
            return false;
        }
        if (f <= this.viewportW2s) {
            return true;
        }
        if (g <= this.viewportH2s) {
            return true;
        }
        var h = f - this.viewportW2s;
        var i = g - this.viewportH2s;
        return h * h + i * i <= c * c;
    }
    isInView(a, b, c) {
    ...
    updateCells(a, c) {
        ...
        c += 2;
        for (var h = 0; h < f; h++) {
            const m = a.readUInt32LE(c);
            const n = this.indexedCells[m];
            const o = a.readUInt32LE(c + 4);
            const p = this.indexedCells[o];
            c += 8;
            if (n && p) {
                p.targetX = n.x;
                p.targetY = n.y;
        ...
        while (true) {
            ...
            c += 4;
            if (i == 0) break;
            let q = this.receiveX(a.readInt32LE(c));
            c += 4;
            let s = this.receiveY(a.readInt32LE(c));
            c += 4;
            const t = a.readUInt16LE(c);
            c += 2;
            const u = a.readUInt8(c++);
            let v = null;
            let w = null;
            let z = '';
            let A = null;
            let B = 0;
            if (u & 128) B = a.readUInt8(c++);
            if (u & 2) {
                const E = a.readUInt8(c++);
                const F = a.readUInt8(c++);
                const G = a.readUInt8(c++);
                v = this.rgb2Hex(~~(E * 0.9), ~~(F * 0.9), ~~(G * 0.9));
            }
            if (u & 4) w = d();
            if (u & 8) {
                z = decodeURIComponent(escape(d()));
            }
            const C = u & 1;
            const D = B & 1;
            if (B & 4) {
            ...
            var k = null;
            if (this.indexedCells.hasOwnProperty(i)) {
                k = this.indexedCells[i];
                if (v) k.color = v;
            } else {
                k = new Cell(i, q, s, t, v, D, C, this.playerCellIDs.indexOf(i) > -1, settings.shortMass, settings.virMassShots);
                k.c = this;
                k.time = this.time;
                if (!D) {
                    if (C && settings.virusesRange) this.viruses.push(k);
                    this.cells.push(k);
                    if (this.playerCellIDs.indexOf(i) > -1 && this.playerCells.indexOf(k) == -1) {
                        k.isPlayerCell = true;
                        this.playerColor = v;
                        this.playerCells.push(k);
                    }
                    if (A) {
                        k.accountID = A;

                        if (this.modeInt !== 3 && !k.isPlayerCell)
                            k.color = 'hsl(' + (A << 314159) % 360 + ',95%,46%)';

                        if (this.cellAID.hasOwnProperty(A)) {
                            this.cellAID[A].push(i);
                        } else {
                            this.cellAID[A] = [i];
                        }
                    }
        ...
        this.updateBound();
        for (const H in this.indexedCells) this.updateStaticBound(this.indexedCells[H]);
        this.calcViewport();
        this.compareCells();
    }
    removeCell(a) {
